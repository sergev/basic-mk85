...
{
    return (unsigned int)ram[addr - RAMSTART] | ((unsigned int)ram[addr + 1 - RAMSTART] << 8);
}
...
{
    return ((uint64_t)read16(addr) << 48) | ((uint64_t)read16(addr + 2) << 32) |
           ((uint64_t)read16(addr + 4) << 16) | (uint64_t)read16(addr + 6);
}
