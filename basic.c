...
int cmd_test(void)
{
    ...
    /* 103C: test the RAM */
    if ((ram_test(0xFF) != RAMSIZE) || (ram_test(0xAA) != RAMSIZE) || (ram_test(0x55) != RAMSIZE) ||
...
void error_handler(int error_code)
{
    ...
    /* 12F0: */
    if ((ram[0x8257u - RAMSTART] & 0x80) != 0 && (ram[0x8256u - RAMSTART] & 0x01) == 0) {
    ...
    if (x != 0) {
        ...
        while (ram[0x8269u - RAMSTART] < SCREEN_WIDTH && (x = ram[dst++ - RAMSTART]) != 0) {
            display_char(x);
        }
...
void msg_px(void)
{
    ...
    display_char('-');
    *int2str(read16(read16(0x825C /*current BASIC line address*/)), &ram[0x816Du - RAMSTART]) = 0;
...
int cmd_defm(void)
{
    ...
    if ((ram[0x8257u /*mode*/ - RAMSTART] & 0x80) != 0 && ram[pc - RAMSTART] == ',') {
        ...
        /* 14E0: write the maximal number of additional variables to the specified
         variable */
        write64(read16(datasp + 2) - 8, int2fp((int16_t)(uint16_t)(free_ram() / 8)));
        datasp += 8;
    }
        ...
        if ((ram[0x8257u /*mode*/ - RAMSTART] & 0x80) != 0 || ram[pc - RAMSTART] != 0) {
            ...
            i += 26;
            if ((unsigned int)i * 8 >
                read16(0x8252 /*top of the RAM*/) - read16(0x823E /*end of BASIC programs*/)) {
                return ERR1;
            }
