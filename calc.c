...
int is_func1 (uint8_t x)
{
/* 1D9C: */ const uint8_t tab1[] =
	{ 0xD2 /* RND */, 0xD3 /* MID */, 0xD4 /* GETC */, 0 };
  int i;
  i = 0;
  while (tab1[i] != x)
  {
...
/* 1ECA: address of the variable */
    write16 (datasp-=2, read16 (0x8252) -
		8 * (unsigned int) (ram[pc++-RAMSTART]-'A')); 
...
/* 1FC2: */
  if ((ram[0x824Bu-RAMSTART] & 0x20) != 0 && ram[0x824Au-RAMSTART] == 0 &&
	(ram[pc-RAMSTART] == 0 || ram[pc-RAMSTART] == ':' ||
	ram[pc-RAMSTART] == ',' || ram[pc-RAMSTART] == 0xE1 /*TO*/ ||
	ram[pc-RAMSTART] == '!'))
  {
    return l1f80 ();
  }
...
int is_term1 (uint8_t x)
{
/* 21D0: */ const uint8_t tab3[] = { ':', ';', '!', 0xE0 /*THEN*/,
	0xE1 /*TO*/, 0xE2 /*STEP*/, 0 };
  int i;
  i = 0;
  while (tab3[i] != x)
