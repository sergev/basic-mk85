...
uint64_t fp_exp(uint64_t x)
{
    ...
    j = 0;
    while (1) {
        b = (j - (int)(int16_t)e < 7) ? ln_tab[j - (int)(int16_t)e] : 0x0009999999500288;
...
uint64_t fp_asn(uint64_t x, unsigned int md /* 0=DEG, 1=RAD, 2=GRA */)
{
    ...
    x = (x > SQRTHALF) ? fp_sub(HALFPI, sub_atn(fp_div(y, x))) : fp_atn(fp_div(x, y), 1);
    x |= s;
    /* 34B8: optional conversion of the angle in RAD to DEG or GRA */
    if (md != 1) {
        x = fp_mul(x, tab5[md]);
...
uint64_t sub_atn(uint64_t a)
{
    ...
    */
    a = 0;
    while (1) {
        b = (j - (int)(int16_t)e < 7) ? atn_tab[j - (int)(int16_t)e] : 0x0010000000000010;
        while ((q & 0xF) != 0) {
            a = bcd_add(a, b);
            --q;
        }
